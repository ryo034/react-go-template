...
func encodeProcessInvitationResponse(response ProcessInvitationRes, w http.ResponseWriter, span trace.Span) error {
	switch response := response.(type) {
	case *ProcessInvitationOK:
		w.WriteHeader(200)
		span.SetStatus(codes.Ok, http.StatusText(200))
...
func encodeVerifyInvitationResponse(response VerifyInvitationRes, w http.ResponseWriter, span trace.Span) error {
	switch response := response.(type) {
	case *InvitationInfo:
		w.Header().Set("Content-Type", "application/json; charset=utf-8")
		w.WriteHeader(200)
